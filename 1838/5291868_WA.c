...
int cmp_by_row(const void* p1, const void* p2)
{
	point_t* a = (point_t*)p1;
	point_t* b = (point_t*)p2;
	return a->x - b->x;
}
int cmp_by_col(const void* p1, const void* p2)
{
	point_t* a = (point_t*)p1;
	point_t* b = (point_t*)p2;
	return a->y - b->y;
...
void init()
{
	...
	qsort((void*)(pos + 1), n, sizeof(pos[1]), cmp_by_row);
	for(i = 1;i <= n; i++)
	{
		if(i - 1 >= 1)
		{
	...
	qsort((void*)(pos + 1), n, sizeof(pos[1]), cmp_by_col);
	for(i = 1;i <= n; i++)
	{
		if(i - 1 >= 1)
		{
