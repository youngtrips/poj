...
void insert(int a, int b, int c, int r)
{
	...
	if(single(seg[r].col))
	{
		seg[r<<1].col = seg[r].col;
		seg[(r<<1) + 1].col = seg[r].col;
	}
	mid = (seg[r].a + seg[r].b)>>1;
	if(a <= mid) insert(a, b, c, r<<1);
	if(b > mid) insert(a, b, c, (r<<1) + 1);
	seg[r].col = seg[r<<1].col | seg[(r<<1) + 1].col;
}
void query(int a, int b, int r)
{
	...
	{
		mid = (seg[r].a + seg[r].b)>>1;
		if(a <= mid) query(a, b, r<<1);
		if(b > mid) query(a, b, (r<<1) + 1);
	}
}
int count(int x)
{
