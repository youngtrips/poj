...
int sap(int st, int ed)
{
	...
	while(dist[st] < n)
	{
		...
		{
			j = e[t].v;
			if(e[t].cap + flow[e[t].u][e[t].v] > 0 && dist[j] + 1 == dist[i])
			{
				found = 1;
				if(e[t].cap + flow[e[t].u][e[t].v]< now_flow) now_flow = e[t].cap + flow[e[t].u][e[t].v];
				pre[j] = t; i = j;
				if(i == ed)
				{
					tot_flow += now_flow;
					while(i != st)
					{
						flow[e[pre[i]].u][e[pre[i]].v] -= now_flow;
						flow[e[pre[i]].v][e[pre[i]].u] += now_flow;
						i = e[pre[i]].u;
					}
					now_flow = oo;
				}
		...
		while(t != -1)
		{
			if(e[t].cap + flow[e[t].u][e[t].v]> 0 && dist[e[t].v] < min) min = dist[e[t].v];
			t = e[t].next;
		}
		dist[i] = min + 1;
		cnt[dist[i]]++;
